...
public class TaskExecFunc {
    ...
    public void createTask(TaskCreateEntity taskCreate) {
        SwingWorker<Void, Void> worker = new SwingWorker<>() {
            ...
            protected Void doInBackground() {
                ...
                sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在计算下载量");
                ...
                if (taskCreate.getIsCoverExists()) {
                    sharedService.pub(SharedType.TASK_STATUS_IS_COVER_EXIST, "是");
                } else {
                    sharedService.pub(SharedType.TASK_STATUS_IS_COVER_EXIST, "否");
                }
                switch (taskCreate.getImgType()) {
                    case 0 -> sharedService.pub(SharedType.TASK_STATUS_IMG_TYPE, "PNG");
                    case 1 -> sharedService.pub(SharedType.TASK_STATUS_IMG_TYPE, "JPG - 低质量");
                    case 2 -> sharedService.pub(SharedType.TASK_STATUS_IMG_TYPE, "JPG - 中等质量");
                    case 3 -> sharedService.pub(SharedType.TASK_STATUS_IMG_TYPE, "JPG - 高质量");
                    default -> {
                    }
                }
                // 生成任务
                TaskAllInfoEntity taskAllInfo = taskStep.tileDownloadTaskCreate(taskCreate, (e) -> {
                    sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, e); // 日志回调
                });
                // 保存下载任务
    ...
    public void loadTask(TaskAllInfoEntity taskAllInfo) {
        SwingWorker<Void, Void> worker = new SwingWorker<>() {
            @Override
            protected Void doInBackground() {
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_CLEAR, true); // 日志回调
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "导入下载任务...");
                List<Integer> zoomList = new ArrayList<>(taskAllInfo.getEachLayerTask().keySet());
                // 显示任务信息
                sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在导入下载任务");
        ...
        SwingWorker<Void, Void> worker = new SwingWorker<>() {
            ...
            protected Void doInBackground() {
                ...
                taskStep.tileDownload(taskAllInfo[0], (z) -> {
                    // 层级下载开始回调
                    sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在下载第" + z + "级地图");
                }, (z) -> {
                    // 层级下载结束回调 - 下载错误瓦片
    ...
    private void eachLayerDownloadedTileMergeCallback(TaskAllInfoEntity taskAllInfo, int z) {
        ...
        if (taskAllInfo.getIsMergeTile() && !taskAllInfo.getEachLayerTask().get(z).getIsMerged()) {
            sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在合成第" + z + "级地图");
            // 声明Mat
    ...
    private void downloadStart() {
        ...
        sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在下载");
    ...
    private void downloadFinish(TaskAllInfoEntity taskAllInfo) {
        ...
        downloadWorker = null;
        sharedService.pub(SharedType.RESOURCE_USAGE_CLEAR, true);
        sharedService.pub(SharedType.DOWNLOAD_CONSOLE_PAUSE_BUTTON_TEXT, "暂停任务");
        ...
        if (!isHandlerCancel) {
            ...
            updateDownloadProcess(new TaskProgressEntity(0, taskAllInfo.getAllRunCount(), (double) taskAllInfo.getAllRunCount() / (double) taskAllInfo.getAllRealCount()));
            sharedService.pub(SharedType.TASK_STATUS_CURRENT, "下载完成");
            sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "下载完成");
            System.out.println("[下载完成]");
            deleteTaskFile(taskAllInfo);
        } else {
            sharedService.pub(SharedType.TASK_STATUS_CURRENT, "下载任务已取消");
            sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "下载任务已取消");
    ...
    public void pauseTask() {
        if (TaskState.IS_TASKING && downloadWorker != null) {
            if (TaskState.IS_TASK_PAUSING) { // 继续
                TaskState.IS_TASK_PAUSING = false;
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_PAUSE_BUTTON_TEXT, "暂停任务");
                sharedService.pub(SharedType.TASK_STATUS_CURRENT, "继续下载");
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "继续下载任务");
            } else { // 暂停
                TaskState.IS_TASK_PAUSING = true;
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_PAUSE_BUTTON_TEXT, "继续任务");
                sharedService.pub(SharedType.TASK_STATUS_CURRENT, "暂停下载");
                sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "已暂停下载任务");
            }
        }
    }
    ...
    public void cancelTaks() {
        if (TaskState.IS_TASKING && downloadWorker != null) {
            sharedService.pub(SharedType.DOWNLOAD_CONSOLE_CANCEL_BUTTON_STATE, true);
            sharedService.pub(SharedType.TASK_STATUS_CURRENT, "正在取消下载任务...");
            sharedService.pub(SharedType.DOWNLOAD_CONSOLE_LOG, "正在取消下载任务...");
