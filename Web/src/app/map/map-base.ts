...
export class MapBase {
    ...
    // 地图DOM
    private mapEle: HTMLDivElement | undefined;
    // 地图实例
	private mapSource: MapSource = new MapSource(ol);
    protected mapObj: any;
    public getOlMap(): any { return this.mapObj };
    // 地图设置
    private mapConfig: MapConfigOption | undefined;
    public setMapConfig(config: MapConfigOption) { this.mapConfig = config; }
    public getMapConfig(): MapConfigOption | undefined { return this.mapConfig; }
    // 当前XYZ图层名
    ...
    constructor(
        config: { dom: HTMLDivElement, option: MapConfigOption },
        callback: { onFinish: Function, onResize?: Function }
    ) {
        this.mapConfig = config.option;
    ...
    private initMap(
        ...
        let mapLayers = [];
        // 底图
        let tilesArray = this.mapSource.layers.get(config.option.layer);
        ...
        this.currentXyzName = config.option.layer;
        // 网格
        let gridLayer = new ol.layer.Tile({
            source: new ol.source.TileDebug(),
            visible: config.option.grid,
            zIndex: 50
        });
        ...
        // 尺寸自适应
        elementResizeDetectorMaker().listenTo(this.mapEle, (e: HTMLElement) => {
            setTimeout(() => {
    ...
    public destroyMap(): void {
        elementResizeDetectorMaker().uninstall(this.mapEle);
    }
    ...
    switchMapResource(name: string): void {
        ...
        // 删除当前图层
        let currArr = this.mapSource.layers.get(this.currentXyzName);
        for (let i = 0; i < currArr.length; i++) {
            let layer = this.getLayerByName(currArr[i].name);
